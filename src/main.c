...
static void print_help(void) {
    ...
    printf("  codebase-memory-mcp hook-memory-session-start\n");
    printf("  codebase-memory-mcp hook-memory-before-tool\n");
    printf("  codebase-memory-mcp hook-memory-after-tool\n");
    printf("  codebase-memory-mcp hook-memory-finalize\n");
...
static int handle_subcommand(int argc, char **argv) {
    ...
    for (int i = SKIP_ONE; i < argc; i++) {
        ...
        if (strcmp(argv[i], "hook-memory-session-start") == 0) {
            cbm_mem_init(MAIN_RAM_FRACTION);
            return cbm_cmd_hook_memory_session_start();
        }
        if (strcmp(argv[i], "hook-memory-before-tool") == 0) {
            cbm_mem_init(MAIN_RAM_FRACTION);
            return cbm_cmd_hook_memory_before_tool();
        }
        if (strcmp(argv[i], "hook-memory-after-tool") == 0) {
            cbm_mem_init(MAIN_RAM_FRACTION);
            return cbm_cmd_hook_memory_after_tool();
        }
        if (strcmp(argv[i], "hook-memory-finalize") == 0) {
            cbm_mem_init(MAIN_RAM_FRACTION);
