...
TEST(cli_agent_instructions_include_amaze_memory_protocol) {
    const char *instr = cbm_get_agent_instructions();
    ASSERT_NOT_NULL(instr);
    ASSERT(strstr(instr, "Amaze Memory Protocol") != NULL);
    ASSERT(strstr(instr, "memory_recall") != NULL);
    ASSERT(strstr(instr, "memory_candidate") != NULL);
    ASSERT(strstr(instr, "memory_validate") != NULL);
...
TEST(cli_memory_hook_context_mentions_protocol) {
    const char *ctx = cbm_get_memory_hook_context("session-start");
    ASSERT_NOT_NULL(ctx);
    ASSERT(strstr(ctx, "AMAZE MEMORY PROTOCOL") != NULL);
    ASSERT(strstr(ctx, "event=session-start") != NULL);
    ASSERT(strstr(ctx, "memory_recall") != NULL);
    ASSERT(strstr(ctx, "memory_candidate") != NULL);
    ASSERT(strstr(ctx, "memory_validate") != NULL);
    ASSERT(strstr(ctx, "memory_commit") != NULL);
    ASSERT(strstr(ctx, "freshness") != NULL);
    PASS();
}

TEST(cli_memory_hook_commands_callable) {
    ASSERT_EQ(cbm_cmd_hook_memory_session_start(), 0);
    ASSERT_EQ(cbm_cmd_hook_memory_before_tool(), 0);
    ASSERT_EQ(cbm_cmd_hook_memory_after_tool(), 0);
    ASSERT_EQ(cbm_cmd_hook_memory_finalize(), 0);
    PASS();
}
...
TEST(cli_memory_config_keys) {
    ASSERT_STR_EQ(CBM_CONFIG_MEMORY_ENFORCEMENT, "memory_enforcement");
    ASSERT_STR_EQ(CBM_CONFIG_MEMORY_FRESHNESS_DAYS, "memory_freshness_days");
    PASS();
}
...
SUITE(cli) {
    ...
    RUN_TEST(cli_agent_instructions_include_amaze_memory_protocol);
    RUN_TEST(cli_memory_hook_context_mentions_protocol);
    RUN_TEST(cli_memory_hook_commands_callable);
    ...
    RUN_TEST(cli_memory_config_keys);
