...
static const char agent_instructions_content[] =
    "# Amaze Agent Memory and Codebase Protocol\n"
    "\n"
    "This project uses Amaze to maintain a codebase knowledge graph and freshness-aware "
    "agent memory.\n"
    "ALWAYS prefer MCP graph tools over grep/glob/file-search for code discovery, and "
    "ALWAYS treat stale memory as untrusted until revalidated from source.\n"
    "\n"
    "## Amaze Memory Protocol\n"
    "1. Start each task by using injected hook context, then call `memory_recall` for "
    "relevant user, project, session, and task memory.\n"
    "2. If recalled memory is stale, conflicting, or source hashes changed, verify with "
    "`memory_search`, `search_graph`, `trace_path`, or `get_code_snippet` before relying "
    "on it.\n"
    "3. Record durable preferences, project facts, decisions, and verified learnings with "
    "`memory_candidate`.\n"
    "4. Promote memory only through `memory_validate` followed by `memory_commit`; do not "
    "store unverified guesses.\n"
    "5. Use `memory_feedback` when recalled memory is useful, ignored, stale, or wrong so "
    "ranking can improve.\n"
    "\n"
    "## Priority Order\n"
    "1. `memory_recall` - load fresh validated memory before planning\n"
    ...
    "7. `memory_candidate`, `memory_validate`, `memory_commit` - capture verified durable "
    "memory after source-backed work\n"
    "\n"
...
const char *cbm_get_memory_hook_context(const char *event_name) {
    static char context[CLI_BUF_4K];
    const char *event = (event_name && event_name[0]) ? event_name : "unknown";
    snprintf(context, sizeof(context),
             "AMAZE MEMORY PROTOCOL\n"
             "event=%s\n"
             "freshness=required\n"
             "backend=manifest-stub\n"
             "required_start: call memory_recall with the current project and task before "
             "planning or editing.\n"
             "required_capture: call memory_candidate for durable user preferences, project "
             "facts, decisions, and verified task learnings.\n"
             "required_validation: call memory_validate before memory_commit; stale or "
             "unverified memories must be refreshed from source.\n"
             "available_tools: memory_recall, memory_search, memory_candidate, "
             "memory_validate, memory_commit, memory_feedback, memory_rebuild, memory_audit, "
...
static int emit_memory_hook_context(const char *event_name) {
    printf("%s", cbm_get_memory_hook_context(event_name));
    return 0;
}
...
int cbm_cmd_config(int argc, char **argv) {
    if (argc == 0) {
        ...
        printf("  %-25s  default=%-10s  %s\n", CBM_CONFIG_MEMORY_FRESHNESS_DAYS, "30",
               "Max age before recalled memory must be revalidated");
        return 0;
    }
    ...
    if (strcmp(argv[0], "list") == 0 || strcmp(argv[0], "ls") == 0) {
        ...
        printf("  %-25s = %-10s\n", CBM_CONFIG_MEMORY_FRESHNESS_DAYS,
               cbm_config_get(cfg, CBM_CONFIG_MEMORY_FRESHNESS_DAYS, "30"));
